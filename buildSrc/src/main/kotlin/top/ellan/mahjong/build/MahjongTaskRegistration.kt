...
object MahjongTaskRegistration {
    ...
    fun registerCodegenTasks(
        ...
        tasks += project.tasks.register("generateMessageIndex") {
            val inputDir = project.layout.projectDirectory.dir("src/main/resources").asFile
        ...
        tasks += project.tasks.register("generateLocalizedConfigs") {
            val templateFile = project.layout.projectDirectory.file("src/main/config-template/config.template.yml").asFile
            val translations = linkedMapOf(
                "config.yml" to project.layout.projectDirectory.file("src/main/config-template/config_comments_en.properties").asFile,
                "config_zh_CN.yml" to project.layout.projectDirectory.file("src/main/config-template/config_comments_zh_CN.properties").asFile,
                "config_zh_TW.yml" to project.layout.projectDirectory.file("src/main/config-template/config_comments_zh_TW.properties").asFile
            )
        ...
        tasks += project.tasks.register("verifyMahjongTileResources") {
            val enumSource = project.layout.projectDirectory.file("src/main/java/top/ellan/mahjong/model/MahjongTile.java").asFile
            val itemsDir = project.layout.projectDirectory.dir("resourcepack/assets/mahjongcraft/items/mahjong_tile").asFile
            val modelsDir = project.layout.projectDirectory.dir("resourcepack/assets/mahjongcraft/models/item/mahjong_tile").asFile
            val texturesDir = project.layout.projectDirectory.dir("resourcepack/assets/mahjongcraft/textures/item/mahjong_tile").asFile
        ...
        tasks += project.tasks.register("generateCraftEngineBundle") {
            val enumSource = project.layout.projectDirectory.file("src/main/java/top/ellan/mahjong/model/MahjongTile.java").asFile
            val resourcepackDir = project.layout.projectDirectory.dir("resourcepack").asFile
            val attributionFile = project.layout.projectDirectory.file("resourcepack/ATTRIBUTION.md").asFile
            inputs.file(enumSource)
            inputs.dir(resourcepackDir)
            inputs.file(attributionFile)
            outputs.dir(File(generatedDir, "craftengine"))
            doLast {
                CraftEngineBundleGenerator.writeCraftEngineBundle(enumSource, resourcepackDir, attributionFile, generatedDir, pluginVersion)
            }
        }
    ...
    fun registerNativeTasks(
        ...
        val configureGbMahjongNative = project.tasks.register<Exec>("configureGbMahjongNative") {
            group = "build"
        ...
        val buildGbMahjongNative = project.tasks.register<Exec>("buildGbMahjongNative") {
            group = "build"
        ...
        tasks += project.tasks.register("packageGbMahjongNative") {
            group = "build"
