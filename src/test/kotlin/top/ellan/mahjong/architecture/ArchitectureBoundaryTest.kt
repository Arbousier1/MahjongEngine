...
class ArchitectureBoundaryTest {
    ...
    fun `known large orchestration files stay under size budgets`() {
        ...
        assertTrue(
            violations.isEmpty(),
            "Large files should shrink or stay flat; raise a budget only " +
                "with an accompanying split plan:\n${violations.joinToString("\n")}",
        )
    }
    ...
    fun `known broad public surfaces stay under method budgets`() {
        val violations =
            publicMethodBudgets.mapNotNull { budget ->
                val file = projectRoot.resolve(budget.path)
                val methodCount =
                    Files
                        .readString(file)
    ...
    private companion object {
        data class LineBudget(
            val path: String,
            val maxLines: Int,
        )

        data class PublicMethodBudget(
            val path: String,
            val maxPublicMembers: Int,
        )

        val projectRoot: Path = Path.of("").toAbsolutePath()
        val forbiddenRuleImports =
            listOf(
                "import org.bukkit.",
                "import io.papermc.",
                "import net.kyori.",
        ...
        fun sourceFiles(vararg roots: String): List<Path> =
            roots.flatMap { root ->
